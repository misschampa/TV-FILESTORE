...
async def incoming_gen_link(bot, message):
    ...
    if WEBSITE_URL_MODE == True:
        share_link = f"{WEBSITE_URL}?EraFileStoreBot={outstr}"
    else:
...
async def gen_link_batch(bot, message):
    ...
    if " " not in message.text:
        return await message.reply("Use correct format.\nExample /batch https://t.me/+7ehnJA3aMb84OGNl https://t.me/ai_image_junction.")
...
# Please see < https://github.com/IamDvis/DV-FILESTORE/blob/master/LICENSE >
    ...
    user = await get_user(user_id)
    if WEBSITE_URL_MODE == True:
        share_link = f"{WEBSITE_URL}?EraFileStoreBot=BATCH-{file_id}"
    else:
